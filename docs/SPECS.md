...
- **En-tête collée** — nom du site (version courte) et recherche, `position: sticky` dès le chargement de la page (comme YouTube, issue #54) : jamais besoin d'un scroll pour les retrouver.
- **Lecteur + sélecteur de livre** (onglets desktop, menu déroulant mobile à venir) — persistants au sens du contenu (le lecteur ne change que sur un clic d'épisode explicite, sommaire ou résultat de recherche, jamais par la seule navigation ; le sélecteur ne pilote que le sommaire affiché, vide la recherche en cours, sans effet sur le lecteur) et au sens visuel : collés *ensemble* juste sous l'en-tête en scrollant le sommaire (issue #54), le lecteur réduit en mini-lecteur avec un repère textuel de l'épisode en cours à côté. Cliquer un épisode ramène en haut, le lecteur reprend sa pleine taille. L'onglet du livre en cours de lecture porte toujours une pastille « en direct », que cet onglet soit affiché ou non (on peut parcourir un livre pendant qu'un autre joue).
