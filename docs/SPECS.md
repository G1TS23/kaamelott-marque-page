...
- **Contrôles de transport** (retour d'usage) — précédent / lecture-pause / suivant, à côté de la mini-timeline, **en plein format seulement** (pas dans le lecteur réduit, trop étroit avec son repère déjà à côté). Précédent/suivant restent dans le livre affiché — jamais de bascule automatique vers le livre suivant/précédent, même limite délibérée que la mini-timeline — bouton désactivé en butée (premier/dernier épisode du livre) plutôt qu'un rebouclage. Le bouton lecture/pause reflète l'état réel du lecteur (pas seulement une intention) : `BUFFERING` y compte comme « en lecture », l'icône ne doit pas clignoter pendant une rebufferisation.
