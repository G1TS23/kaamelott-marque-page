...
- **Lecteur + sélecteur de livre** (onglets desktop, menu déroulant mobile à venir) — persistants au sens du contenu (le lecteur ne change que sur un clic d'épisode explicite, sommaire ou résultat de recherche, jamais par la seule navigation ; le sélecteur ne pilote que le sommaire affiché, vide la recherche en cours, sans effet sur le lecteur) et au sens visuel, mais seulement quand une vidéo *joue réellement* (pas une simple vignette affichée ni une vidéo en pause, issue #54) : collés *ensemble* juste sous l'en-tête en scrollant le sommaire, le lecteur réduit en mini-lecteur avec un repère textuel de l'épisode en cours à côté. Cliquer un épisode ramène en haut, le lecteur reprend sa pleine taille ; mettre en pause décolle immédiatement le groupe. L'onglet du livre en cours de lecture porte toujours une pastille « en direct », que cet onglet soit affiché ou non (on peut parcourir un livre pendant qu'un autre joue).
