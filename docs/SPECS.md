...
- **Lien profond partageable** (issue #21) : `?livre=1&episode=s1e02` (l'`id` de la fiche, pas le seul numéro d'épisode — sans le livre, un numéro n'identifie rien, section 4) ouvre le bon livre et positionne le lecteur à la bonne seconde dès sa construction (`playerVars.start`, pas `loadVideoById` — la vidéo initiale du lecteur est déjà la bonne, inutile de la recharger), puis tente de lancer la lecture. Si le navigateur bloque l'autoplay non muet — le cas le plus courant sur une première visite via un lien partagé, sans interaction préalable avec le site — la vidéo reste positionnée à la bonne seconde, il ne reste qu'à presser lecture ; si l'autoplay passe, elle démarre directement. Bouton « copier le lien » à côté des contrôles de transport (en plein format seulement, même contrainte de place) pour générer ce lien depuis l'épisode en cours.
...
- **Intro par livre, épisode `0` synthétique** (issue #71) : la portion de la vidéo avant le premier épisode réel est exposée comme un pseudo-épisode « Intro », généré au chargement des données (jamais stocké dans `data/episodes/livre-N.json`) — sert de position par défaut du lecteur avant tout engagement réel, comble le trou où le sommaire et le repère sous le lecteur n'avaient rien à afficher. Cliquable dans le sommaire et partageable par lien profond (`?livre=1&episode=s1e00`), mais exclu de la recherche par titre (générique, sans résumé ni personnage propre).
...
- **En-tête collée** — nom du site (version courte, remplace le grand titre) et recherche, toujours collée dès le chargement de la page et sur toute la largeur de la fenêtre (comme YouTube, issue #54) : jamais besoin d'un scroll pour les retrouver.
- **Lecteur + sélecteur de livre** (onglets, libellé court sous 640px pour tenir sur une seule ligne) — persistants au sens du contenu (le lecteur ne change que sur un clic d'épisode explicite, sommaire ou résultat de recherche, jamais par la seule navigation ; le sélecteur ne pilote que le sommaire affiché, vide la recherche en cours, sans effet sur le lecteur) et au sens visuel, mais seulement quand une vidéo *a été réellement lancée* (pas une simple vignette jamais démarrée, issue #54) : collés *ensemble* juste sous l'en-tête en scrollant le sommaire, le lecteur réduit en mini-lecteur avec un repère textuel à côté (livre, puis numéro et titre de l'épisode, issue #56) — le même repère s'affiche aussi sous le lecteur en plein format, pas seulement une fois réduit, jamais les deux à la fois. Une pause ne décolle pas le groupe — seule une vidéo jamais lancée, ou revenue à cet état, le fait. Cliquer un épisode ramène en haut, le lecteur reprend sa pleine taille. Le repère et la ligne surlignée du sommaire suivent la position de lecture réelle (`getCurrentTime`, sondée en continu), pas seulement le dernier épisode cliqué : ils changent aussi si la vidéo avance jusqu'à l'épisode suivant, ou si on saute dans la barre de progrès YouTube (issue #56). L'onglet du livre en cours de lecture porte toujours une pastille « en direct », que cet onglet soit affiché ou non (on peut parcourir un livre pendant qu'un autre joue).
- **Mini-timeline** (issue #56) — sous le lecteur (pleine taille comme réduit) dès qu'une lecture est engagée : barre d'avancement bornée à l'épisode en cours seulement (début de cet épisode → début du suivant, ou fin de la vidéo pour le dernier épisode d'un livre), cliquable/glissable à la souris comme au clavier (flèches, Origine/Fin), temps écoulé affiché en `m:ss` — contrairement à la timeline YouTube native, qui couvre toute la vidéo (plusieurs heures) et n'aide pas à se repérer *dans* un épisode précis. Un dépassement de la borne haute (par lecture normale ou par ce geste) fait basculer sur l'épisode suivant, comme pour le repère ci-dessus. Un texte « Chargement… » recouvre le lecteur pendant l'état `BUFFERING` de l'API IFrame : un saut vers un point jamais bufferisé d'une vidéo de plusieurs heures peut prendre plusieurs secondes (écran noir sinon, pas un bug, une latence réseau/CDN normale mais invisible sans ce retour).
- **Contrôles de transport** (retour d'usage) — précédent / lecture-pause / suivant, dans les deux formats mais pas au même endroit : à côté de la mini-timeline en plein format, sous le repère en réduit (pas assez de place à côté d'un mini-lecteur de 10rem pour trois boutons de plus, la colonne du repère si). Précédent/suivant restent dans le livre affiché — jamais de bascule automatique vers le livre suivant/précédent, même limite délibérée que la mini-timeline — bouton désactivé en butée (premier/dernier épisode du livre) plutôt qu'un rebouclage. Le bouton lecture/pause reflète l'état réel du lecteur (pas seulement une intention) : `BUFFERING` y compte comme « en lecture » (au clic comme à l'affichage de l'icône), l'icône ne doit pas clignoter pendant une rebufferisation.
- **Sommaire** — liste des épisodes du livre affiché, visible par défaut quand la recherche est vide.
- **Pied de page** — toujours visible en bas de l'écran (`position: fixed`), une seule ligne même en mobile. Crédit permanent à Shisheyu (chaînes YouTube et Twitch), lien vers le site et le GitHub de l'auteur du projet, lien vers les mentions légales (issue #20, périmètre redéfini en cours de route — la version d'origine liait la vidéo source et la page Wikipédia du livre actif, retirées sur retour d'usage).
...
- Requête vide → sommaire du livre actif. Requête non vide → résultats globaux aux 4 livres. **Correction post-implémentation (issue #16)** : plus de remplacement en place du sommaire — les résultats vivent dans le panneau unifié (`ResultatsRecherche.svelte`), le sommaire ne montre plus que le livre actif. Badge « Livre N » affiché sur **tous** les résultats, pas seulement ceux d'un autre livre que l'onglet affiché (retour d'usage : plus simple à lire d'un coup d'œil que de faire deviner le livre affiché par son absence).
...
- Recherche globale aux quatre livres par défaut, badge « Livre N » sur tous les résultats (pas seulement ceux hors du livre affiché — retour d'usage post-implémentation).
